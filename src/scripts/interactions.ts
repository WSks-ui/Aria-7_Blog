...
export const initInteractions = (providedScope?: PageScope): (() => void) | undefined => {
  ...
  if (labFeed && !reduceMotion) {
    const feedSpot = labFeed.querySelector("[data-feed-spot]");
    ...
    const syncFeedPointer = () => {
      ...
      // 光斑是独立小元素，transform 只触发合成；translate(-50%,-50%) 使其居中于指针。
      if (feedSpot) {
        runtimeStyles.set(
          feedSpot,
          "transform",
          `translate3d(${Math.round(x * bounds.width)}px, ${Math.round(y * bounds.height)}px, 0) translate(-50%, -50%)`,
        );
      }
    };
  ...
  // 更新热力图：代码/写作双模式。快照缺失时组件只渲染写作面板，按钮不出现，此逻辑自然空转。
  const heatmap = document.querySelector("[data-heatmap]");
  if (heatmap) {
    const modeButtons = [...heatmap.querySelectorAll("[data-heatmap-mode]")];
    const panels = [...heatmap.querySelectorAll("[data-heatmap-panel]")];
    const storageKey = "aria-heatmap-mode";

    const syncHeatmapMode = (mode: string, shouldUpdateUrl = true) => {
      const fallback = heatmap.dataset.heatmapDefault || "writing";
      const selected = panels.some((panel) => panel.dataset.heatmapPanel === mode) ? mode : fallback;

      modeButtons.forEach((button) => {
        button.setAttribute("aria-pressed", String(button.dataset.heatmapMode === selected));
      });
      panels.forEach((panel) => {
        panel.toggleAttribute("hidden", panel.dataset.heatmapPanel !== selected);
      });

      storage.set(storageKey, selected);
      if (shouldUpdateUrl) {
        const url = new URL(window.location.href);
        if (selected === fallback) url.searchParams.delete("heatmap");
        else url.searchParams.set("heatmap", selected);
        window.history.replaceState({}, "", `${url.pathname}${url.search}${url.hash}`);
      }
    };

    modeButtons.forEach((button) => {
      onPage(button, "click", () => syncHeatmapMode(button.dataset.heatmapMode || ""));
    });

    const params = new URLSearchParams(window.location.search);
    syncHeatmapMode(params.get("heatmap") || storage.get(storageKey) || "", false);
  }
  ...
  // 标签物理引擎与点击计数器已随首页重构移除（标签改为静态 dock，计数器无对应标记）；
  // Dock/音乐的历史注释实现也已删除，正式实现见 features/dock-controller.ts 与 features/music-controller.ts。
  return cleanup;
};
